...
const checkToken = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (validToken.data.valid == false)
        return new Error('Invalid token. rerun init() to replace with a valid token');
    else {
        return true;
    }
});
const arrCheck = (arrToCheck) => {
    for (let i = 0; i < arrToCheck.length; i++) {
        if (typeof arrToCheck[i] == 'string' && arrToCheck[i].trim() == '')
            throw new Error('Insufficient or empty arguements');
        if (!arrToCheck[i])
            throw new Error('Insufficient or empty credetials');
    }
};
// main class
/**
    ...
    upsertUser(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.token || (typeof this.token == 'string' && this.token.trim() == ''))
                throw new Error('Empty token. Run init() first');
            if (!id)
            ...
            if (yield checkToken(this)) {
                ...
                return new Error('Invalid Token, run init()');
        });
    }
    /**
